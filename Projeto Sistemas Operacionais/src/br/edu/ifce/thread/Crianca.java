...
public class Crianca extends Thread {
        ...
        while(true) { //Loop infinito para que o método run nunca termine sua execução.
            if(this.tenho_bola){
                try { //O uso do método acquire da classe Semaphore exige um try/catch
                    ...
                    /*
                     * Não sei o que esse catch faz, pois foi adicionado automaticamente pelo sistema.
                     * Mas se quiserem personalizar, podem colocar um JOptionPane informando ao usuário que o sistema lançou um erro
                     * durante o uso do método acquire e solicite-o que finalize e execute novamente a aplicação.
                     */
                    Logger.getLogger(Crianca.class.getName()).log(Level.SEVERE, null, ex);
                } finally { //Este bloco finally pode ser eliminado e seu conteúdo ficar dentro do try, logo após o método devolverBolaParaCesto() na linha 89;
                    /*
                    ...
                    this.quieta(this.tempoQuieta); //Após brincar e devolver a bola para o cesto, a criança ficará quieta por um determinado tempo.
                }
            //Fim if(tenhoBola);
    ...
    public void brincar(long unidadeDeTempo) {
        ...
        do {
            this.telaPrincipal.mostrarCriancaBrincando_estado1(this.idCrianca, this.nome); //Mostra a imagem da criança com bola perto da cabeça;
            finish = System.currentTimeMillis(); // Recebe novamente a hora atual do sistema
            this.telaPrincipal.mostrarCriancaBrincando_estado2(this.idCrianca, this.nome); //Mostra criança a imagem da criança com a bola distante da cabeça;
